...
import { RefreshCw, TrendingUp, Users, MousePointer, DollarSign, Globe, Calendar, Download, CreditCard, Settings, CheckCircle, AlertCircle, Clock, Crown, Zap, Shield } from 'lucide-react'

// Complete Enhanced Dashboard with ALL Features:
// ✅ Phase 2 Real-time Analytics (1,493 lines preserved)
// ✅ Complete Payment System Integration
// ✅ All New Backend API Endpoints
// ✅ Phase 1 + Phase 2 + Payment functionality
...
const EnhancedDashboard = () => {
  ...
  // Check authentication status and load payment data
  useEffect(() => {
  ...
  const requestPayout = async (amount) => {
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        // Refresh payout history and user balance
        loadUserProfile()
        loadPayoutHistory()
      } else {
    ...
    <div className="min-h-screen bg-gray-50 p-6 pt-24">
      <div className="max-w-7xl mx-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          {/* NEW: Billing Tab - Payment System Integration */}
          <TabsContent value="billing" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Manage your CookieBot.ai subscription</CardDescription>
              </CardHeader>
              <CardContent>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-3">
                    <div>
                      ...
                      <Progress 
                        value={usage.limits?.website_limit ? (usage.usage?.websites_count || 0) / usage.limits.website_limit * 100 : 0} 
                        className="h-2"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Progress 
                        value={usage.limits?.api_call_limit ? (usage.usage?.api_calls || 0) / usage.limits.api_call_limit * 100 : 0} 
                        className="h-2"
                      />
                    </div>
                  </div>
              ...
              <CardContent>
                {payoutMethods.length > 0 ? (
                      ...
                      <div key={method.id} className="flex items-center justify-between p-3 border rounded-lg">
                        <div className="flex items-center space-x-3">
                          <div className="p-2 bg-gray-100 rounded">
                            {method.provider === 'stripe' ? <CreditCard className="h-4 w-4" /> : <DollarSign className="h-4 w-4" />}
                          </div>
                          <div>
                            <p className="font-medium">{method.provider.charAt(0).toUpperCase() + method.provider.slice(1)}</p>
                            <p className="text-sm text-gray-600">{method.account_id}</p>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                  ...
                  <div className="text-center py-6">
                    ...
                    <p className="text-gray-600 mb-4">No payout methods configured</p>
          ...
          <DialogContent className="max-w-4xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 py-6">
              {subscriptionPlans.map((plan) => (
                <Card key={plan.name} className={`relative ${currentSubscription?.tier === plan.name ? 'ring-2 ring-blue-500' : ''}`}>
                  <CardHeader>
                    <CardTitle className="flex items-center justify-between">
                      {plan.name.charAt(0).toUpperCase() + plan.name.slice(1)}
                      {plan.name === 'pro' && <Badge className="bg-gradient-to-r from-blue-600 to-purple-600 text-white">Popular</Badge>}
                    </CardTitle>
                    <CardDescription>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center">
                        ...
                        <span>{plan.website_limit === -1 ? 'Unlimited' : plan.website_limit} websites</span>
                      </div>
                      <div className="flex items-center">
                        <CheckCircle className="h-4 w-4 text-green-500 mr-2" />
                        <span>{plan.api_call_limit === -1 ? 'Unlimited' : plan.api_call_limit.toLocaleString()} API calls</span>
                      </div>
                      <div className="flex items-center">
                        <CheckCircle className="h-4 w-4 text-green-500 mr-2" />
                        <span>{plan.revenue_share * 100}% revenue share</span>
                      </div>
                    ...
                    <Button 
                      className="w-full" 
                      disabled={currentSubscription?.tier === plan.name || paymentLoading}
                      onClick={() => handleUpgrade(plan.name)}
                    >
                      {currentSubscription?.tier === plan.name ? 'Current Plan' : 
